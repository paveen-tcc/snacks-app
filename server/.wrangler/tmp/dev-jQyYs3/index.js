...
var PgDialect = class {
  ...
  buildSelectQuery({
    ...
    for (const f of fieldsList) {
      if (is2(f.field, Column) && getTableName(f.field.table) !== (is2(table3, Subquery) ? table3._.alias : is2(table3, PgViewBase) ? table3[ViewBaseConfig].name : is2(table3, SQL) ? void 0 : getTableName(table3)) && !((table22) => joins?.some(
...
var hotDrinks = pgTable("hot_drinks", {
  id: uuid("id").primaryKey().defaultRandom(),
...
var drinkVotes = pgTable("drink_votes", {
  id: uuid("id").primaryKey().defaultRandom(),
  userId: uuid("user_id").references(() => users.id).notNull(),
  date: date("date").notNull(),
...
var shutdownDays = pgTable("shutdown_days", {
  id: uuid("id").primaryKey().defaultRandom(),
  date: date("date").unique().notNull(),
...
var snackRoutes = new Hono2();
snackRoutes.get("/", async (c) => {
  try {
    const db = c.get("db");
    const allSnacks = await db.select().from(snacks).where(eq(snacks.isActive, true)).orderBy(desc(snacks.sortOrder), snacks.name);
    return c.json({ snacks: allSnacks }, 200);
  } catch (err) {
    return c.json({ error: err.message }, 500);
  }
});
var snacks_default = snackRoutes;
...
orderRoutes.get("/status", async (c) => {
  try {
    ...
    const [holiday] = await db.select().from(holidays).where(sql`${holidays.date} = ${today}`).limit(1);
    const [shutdown] = await db.select().from(shutdownDays).where(sql`${shutdownDays.date} = ${today}`).limit(1);
    if (holiday) {
...
orderRoutes.get("/today", async (c) => {
  try {
    ...
    const user = c.get("user");
    const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    const [order] = await db.select().from(orders).where(and(eq(orders.userId, user.userId), sql`${orders.date} = ${today}`)).limit(1);
    if (!order) {
...
orderRoutes.post("/", async (c) => {
  try {
    const db = c.get("db");
    const user = c.get("user");
    const { snackId, date: date2 } = await c.req.json();
    const orderDate = date2 || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    if (!snackId) {
    ...
    const [savedOrder] = await db.insert(orders).values({
      ...
      set: { snackId, updatedAt: /* @__PURE__ */ new Date() }
    }).returning();
...
orderRoutes.get("/history", async (c) => {
  try {
    ...
    const history = await db.select().from(orders).where(
      and(
        and(
...
var drinkRoutes = new Hono2();
drinkRoutes.get("/", async (c) => {
  try {
    const db = c.get("db");
    const drinks = await db.select().from(hotDrinks).where(eq(hotDrinks.isActive, true));
...
drinkRoutes.get("/vote", async (c) => {
  try {
    ...
    const user = c.get("user");
    const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    const [vote] = await db.select().from(drinkVotes).where(and(eq(drinkVotes.userId, user.userId), sql`${drinkVotes.date} = ${today}`)).limit(1);
...
drinkRoutes.post("/vote", async (c) => {
  try {
    const db = c.get("db");
    const user = c.get("user");
    const { drinkId, date: date2 } = await c.req.json();
    const voteDate = date2 || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    if (!drinkId) {
      return c.json({ error: "drinkId is required" }, 400);
    }
    const [savedVote] = await db.insert(drinkVotes).values({
      userId: user.userId,
      date: voteDate,
      drinkId
    }).onConflictDoUpdate({
      target: [drinkVotes.userId, drinkVotes.date],
      set: { drinkId, votedAt: /* @__PURE__ */ new Date() }
    }).returning();
...
drinkRoutes.get("/results", async (c) => {
  try {
    const db = c.get("db");
    const dateStr = c.req.query("date") || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    const results = await db.select({
      drinkId: drinkVotes.drinkId,
      drinkName: hotDrinks.name,
      voteCount: sql`count(${drinkVotes.id})`.mapWith(Number)
    }).from(drinkVotes).innerJoin(hotDrinks, eq(drinkVotes.drinkId, hotDrinks.id)).where(sql`${drinkVotes.date} = ${dateStr}`).groupBy(drinkVotes.drinkId, hotDrinks.name);
    return c.json({ date: dateStr, results }, 200);
  } catch (err) {
    return c.json({ error: err.message }, 500);
  }
});
var drinks_default = drinkRoutes;
...
adminRoutes.put("/snacks/:id", async (c) => {
  try {
    ...
    const [updatedSnack] = await db.update(snacks).set(updateData).where(eq(snacks.id, id)).returning();
...
adminRoutes.put("/settings", async (c) => {
  try {
    ...
    const [setting] = await db.insert(appSettings).values({ key, value: String(value) }).onConflictDoUpdate({
...
adminRoutes.get("/summary", async (c) => {
  try {
    ...
    }).from(orders).innerJoin(snacks, eq(orders.snackId, snacks.id)).where(sql`${orders.date} = ${today}`).groupBy(orders.snackId, snacks.name, snacks.emoji);
    const drinkCounts = await db.select({
      drinkId: drinkVotes.drinkId,
      drinkName: hotDrinks.name,
      drinkEmoji: hotDrinks.emoji,
      count: sql`count(*)`.mapWith(Number)
    }).from(drinkVotes).innerJoin(hotDrinks, eq(drinkVotes.drinkId, hotDrinks.id)).where(sql`${drinkVotes.date} = ${today}`).groupBy(drinkVotes.drinkId, hotDrinks.name, hotDrinks.emoji);
...
adminRoutes.post("/users/:id/admin", async (c) => {
  try {
    ...
    const [updatedUser] = await db.update(users).set({ isAdmin }).where(eq(users.id, id)).returning();
...
adminRoutes.get("/users", async (c) => {
  try {
    const db = c.get("db");
    const allUsers = await db.select({ id: users.id, username: users.username, email: users.email, isAdmin: users.isAdmin, createdAt: users.createdAt }).from(users).orderBy(users.createdAt);
...
adminRoutes.post("/holidays", async (c) => {
  try {
    const db = c.get("db");
    const user = c.get("user");
    const { date: date2, name } = await c.req.json();
    if (!date2) return c.json({ error: "date is required" }, 400);
    const [holiday] = await db.insert(holidays).values({ date: date2, name, source: "manual", createdBy: user.userId }).returning();
...
app.route("/api/snacks", snacks_default);
app.route("/api/orders", orders_default);
app.route("/api/drinks", drinks_default);
...
// .wrangler/tmp/bundle-DfyjTo/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-DfyjTo/middleware-loader.entry.ts

...
A separate `server/AGENTS.md` exists with Bun-specific instructions (use `bun` over `node`/`npm`/`pnpm`, `bun test`, `Bun.serve`, etc.) — follow it when working inside `server/`.
